 ...
 * 双向链表保存每个节点的“新旧程度”，这边自己采取的是“新”节点放到链表的末尾，所以可以通过头结点直接获取到最“旧”的节点
 * 
 * get操作的时间复杂度为 O(1)
 * put操作的时间复杂度为 O(1)
 *
 * 空间复杂度: 由于需要使用一个散列表保存key与Node的映射关系，所以需要额外的 capacity 个 key-Node 节点的存放空间
 * 所以空间复杂度为O(n),其中n为capacity
 * 
 */
